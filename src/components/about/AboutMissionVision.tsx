...
export default function AboutMissionVision() {
  ...
  return (
    ...
    >
      {/* Subtle grid bg */}
      <div className="pointer-events-none absolute inset-0 [mask-image:linear-gradient(to_bottom,transparent,black 10%,black 90%,transparent)]">
      ...
      <div className="mx-auto max-w-6xl px-6 py-28 md:py-36 space-y-32">
        {points.map((item, idx) => (
          <article
            key={item.id}
            id={item.id}
            className="amv-section grid min-h-[70vh] grid-cols-1 items-center gap-10 md:grid-cols-2"
          >
            {/* LEFT */}
            <div className="relative z-10">
              <span className="amv-kicker mb-3 inline-block text-xs tracking-[0.3em] text-red-500/90">
                {idx === 0 ? "WHO WE ARE" : idx === 1 ? "WHAT DRIVES US" : "WHERE WE'RE HEADED"}
              </span>
...
function Badge({ label, className = "" }: { label: string; className?: string }) {
  ...
  useLayoutEffect(() => {
    ...
    // ✅ Explicit void return for cleanup
    return () => { 
      tl.kill(); 
    };
  }, []);
